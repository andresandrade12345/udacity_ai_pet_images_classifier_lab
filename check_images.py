...
def get_pet_labels(image_dir):
    ...
    for i in range(0, len(filename_list)):
        ...
        if len(pet_label):
            petlabels_dic[filename] = pet_label
...
def classify_images(images_dir, petlabel_dic, model):
    ...
    for filename, pet_label in petlabel_dic.items():
        ...
        model_label = classifier(filepath, model).lower().strip()
        
        found_idx = model_label.find(pet_label)
        
        if found_idx >= 0:
            if ((found_idx == 0 and len(pet_label) == len(model_label)) or 
                (((found_idx == 0) or (model_label[found_idx - 1] == " ")) and 
                 ((found_idx + len(pet_label) == len(model_label)) or 
                  (model_label[found_idx + len(pet_label) :found_idx + len(pet_label) + 1] in (" ",","))
                 )
                )
               ):
                if filename not in results_dic:
                    results_dic[filename] = [pet_label, model_label, 1]
            else:
                if filename not in results_dic:
                    results_dic[filename] = [pet_label, model_label, 0]
        else:
            if filename not in results_dic:
                results_dic[filename] = [pet_label, model_label, 0]
    ...
    with open(dogsfile) as file:
        ...
        while line != '':
            ...
            if line not in dognames_dic:
                ...
                print('Warning: Duplicated names', line)
...
def calculates_results_stats(results_dic):
    ...
    results_stats['n_notdogs_images'] = (results_stats['n_images'] - results_stats['n_dogs_images'])
    
    results_stats['pct_match'] = (results_stats['n_match'] / results_stats['n_images']) * 100.0
    results_stats['pct_correct_dogs'] = (results_stats['n_correct_dogs'] / results_stats['n_dogs_images']) * 100.0
    results_stats['pct_correct_breed'] = (results_stats['n_correct_breed'] / results_stats['n_dogs_images']) * 100.0
    
    if results_stats['n_notdogs_images'] > 0:
        results_stats['pct_correct_notdogs'] = (results_stats['n_correct_notdogs'] / results_stats['n_notdogs_images']) * 100.0
    else:
...
def print_results(results_dic, results_stats, model, print_incorrect_dogs = False, print_incorrect_breed = False):
    ...
    print('%20s: %3d' % ('Number of Images', results_stats['n_images']))
    print('%20s: %3d' % ('Number of Dog Images', results_stats['n_dogs_images']))
    print('%20s: %3d' % ('Number of "Not-a" Dog Images: ', results_stats['n_notdogs_images']))
    ...
    if (print_incorrect_dogs and 
        ...
        for key in results_dic:
            if sum(results_dic[key][3:]) == 1:
                print('Real: %-26s  Cassifier: %-30s' % (results_dic[key][0], results_dic[key][1]))
